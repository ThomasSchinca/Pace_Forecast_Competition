...
class finder():
    ...
    def predict_norm(self,horizon=14,plot=True,metric='euclidean',min_d=0.5,dtw_sel=0,select=True):
        """
    ...
    def predict(self,horizon=14,plot=True,mode='mean',seq_out=False):
    ...
    def create_sce(self,horizon=14):
        if len(self.sequences) == 0:
            raise 'No shape found, please fit before predict.'
    ...
    def create_sce_predict(self,horizon=14):
        if len(self.sequences) == 0:
            raise 'No shape found, please fit before predict.'
        tot_seq = [[series.name, series.index[-1],series.min(),series.max(),series.sum()] for series, weight in self.sequences]
        pred_seq=[]
    ...
    def predict_distrib(self,horizon=14,mode='poisson',quantile=[0.05,0.95]):
    ...
    def predict_best_sce(self,horizon=14,div_h=3):
        if len(self.sequences) == 0:
            raise 'No shape found, please fit before predict.'
        tot_seq = [[series.name, series.index[-1],series.min(),series.max(),series.sum()] for series, weight in self.sequences]
        pred_seq=[]
        ...
        tot_seq=pd.DataFrame(pred_seq)
        linkage_matrix = linkage(tot_seq, method='ward')
        clusters = fcluster(linkage_matrix, horizon/div_h, criterion='distance')
        tot_seq['Cluster'] = clusters
        value_counts = tot_seq['Cluster'].value_counts()
        max_count = value_counts.max()
        top_clusters = value_counts[value_counts == max_count].index
        filtered_tot_seq = tot_seq[tot_seq['Cluster'].isin(top_clusters)]
        filtered_tot_seq=filtered_tot_seq.iloc[:,:-1]
        return filtered_tot_seq
